...
KILO_BASE_URL = "https://api.kilo.dev/v1"
...
def _upload_to_supabase_storage(local_path: Path, remote_path: str, content_type: str = "application/octet-stream") -> str:
    sb_client.storage.from_("assets").upload(remote_path, local_path.read_bytes(), {"content-type": content_type})
...
async def generate_manifest_endpoint(req: GenerateManifestRequest):
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=120.0) as client:
            response = await client.post(
                ...
                headers={
                    "Authorization": f"Bearer {os.getenv('KILO_API_KEY')}",
                    ...
                    "messages": [
                        {
                            ...
                            "content": (
                                ...
                                "Generate a scene_manifest with scenes array. "
        ...
        async with httpx.AsyncClient(timeout=120.0) as client:
            for scene in manifest.get("scenes", []):
                ...
                tts_response = await client.post(
                    ...
                    headers={
                        "api-key": os.getenv("MIMO_TTS_API_KEY", ""),
                        "Content-Type": "application/json",
                    },
                    json={
                        "model": "mimo-v2.5-tts",
        ...
        # Stage 4 — NVIDIA NIM Fallback & Upload (progress: 90)
        async with httpx.AsyncClient(timeout=120.0) as client:
            for scene in manifest.get("scenes", []):
